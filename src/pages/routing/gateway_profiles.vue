  ...
  <section>
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.gatewayName" placeholder="Gateway Name"></el-input>
        </el-form-item>
        <el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getObjects() {
        const params = {
          'filter[where][domain_name]': this.filters.gatewayName,
        };
        this.listLoading = true;
        // NProgress.start();
        new GatewayProfileProxy(params).findAll().then((response) => {
      ...
      handleDel(index, row) {
        this.$confirm('Are you sure?', 'prompt', {
          ...
          // NProgress.start();
          const obj = { id: row.id };
          new GatewayProfileProxy().destroy(obj.id).then((response) => {
            this.listLoading = false;
            // NProgress.done();
            this.$message({
              message: 'Success',
      ...
      handleViewScript(index, row) {
        // row.domainName;
        Vue.router.push({ name: 'Gateway Profile Scripts', params: { id: row.id } });
      },
        ...
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            this.$confirm('Are you sure?', 'prompt', {}).then(() => {
              ...
              new GatewayProfileProxy().update(obj.id, obj).then((response) => {
